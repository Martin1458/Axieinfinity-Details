...
pathToNewCsv ='/home/martin/Desktop/pythonShit/AxieInfinity/AxieSmallDetails.csv'
...
compress = 1000
h = 0
csvRows = len(axieCsv.index) // 1000
csvRowsLeft = len(axieCsv.index) - (csvRows * 1000)
...
for i in range(1):
    for item in whatToConvert:
        ...
        for e in range(csvRows):
            if (compress*e)+compress > (csvRows*1000):
...
def deleteIt():
    open('AxieSmallDetails.csv', 'w').close()
